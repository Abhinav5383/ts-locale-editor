...
export function AssembleTsTranslation(props: AssembleTranslationProps): string | null {
	...
	for (const exportItem of sortedExports) {
		...
		if (!identifier) {
			console.warn("Could not determine export identifier for export item. Skipping.", exportItem);
			continue;
		}
	...
	const remainingNodes = sortedTranslatedNodes.value.filter((node) => !assembledTranslationNodes.includes(node.key));
...
function stringifyFunctionNode(node: FunctionNode, indent = 0, exportType = ExportType.Default): string {
	let paramStr = "";
