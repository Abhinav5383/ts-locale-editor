...
import { type FunctionNode, NodeType, type TranslationFn_BlockExprBody, type TranslationNode } from "~/lib/types";
...
export function FunctionRenderer(props: NodeRendererProps<FunctionNode>) {
	...
	return (
		<Switch>
			<Match keyed when={props.node.body.type === NodeType.BlockExpression && props.node.body}>
				{(fnBody) => (
					<div class="node-function block-func">
						...
						>
							<ContentEditable value={fnBody.value} onChange={handleBlockFnChange} className="code-editable" />
						</Show>
						<div>
							<span class="token token-punctuation">{"}"}</span>
