...
export function flattenLocaleEntries(
	...
	for (let i = 0; i < orderedKeys.length; i++) {
		...
		let editNode = editNodeIndex !== undefined ? translatingLocaleNode.value[editNodeIndex] : undefined;
		...
		if (refNode.type === NodeType.Object && editNode.type === NodeType.Object) {
			...
			const ignoreDepth = key === ExportType.Default && parentPath.length === 0 && _uniqueKeys.size === 1;
